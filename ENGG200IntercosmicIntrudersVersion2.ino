...
const int JOYSTICK_X_THRESHOLD = 10;
...
int resetCount = 0;
  ...
  // Draw the player
  tft.fillRect(playerX, playerY, PLAYER_WIDTH, PLAYER_HEIGHT, TFT_BLUE);

  // Update the player position based on joystick input
  if (joystickX < 512 - JOYSTICK_X_THRESHOLD) {
    playerX = max(playerX - 50, 0);
  }
  else if (joystickX > 512 + JOYSTICK_X_THRESHOLD) {
    playerX = min(playerX + 50, SCREEN_WIDTH - PLAYER_WIDTH);
  }

  // Draw the falling rectangle
  tft.fillRect(rectangleX, rectangleY, RECTANGLE_WIDTH, RECTANGLE_HEIGHT, TFT_RED);
